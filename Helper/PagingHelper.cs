...
namespace OrderBento.Helper
{
    public class PagingHelper
    {
        ...
        {
            string gpName = HttpContext.Current.Request.QueryString["GpName"];
            string restName = HttpContext.Current.Request.QueryString["RestName"];

            List<string> conditions = new List<string>();

            if (!string.IsNullOrEmpty(gpName))
                conditions.Add("GpName=" + gpName);

            if (!string.IsNullOrEmpty(restName))
                conditions.Add("RestName=" + restName);
